...
```{r, using_the_ideas6}
## 2 ##
out <- lapply(1:length(years), function(yrIdx) {
     sub <- subset(gap, gap$year == years[yrIdx])
     # fit regression
     return(lm(lifeExp ~ log(gdpPercap), data = sub))
})
...
Since operators are functions, this says to take each element of the `params` list and run the function `[[`(params[[i]], 1)`, which is equivalent to doing:
...
## 4 ##
gap_wider <- output %>% select(country, year, gdpPercap_diff) %>%
