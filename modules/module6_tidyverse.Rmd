...
Note that if you use an aggregation function such as `mean()` within `mutate()` without using `groupby()`, you'll simply do the summary over all the rows of the input data frame and assign the resulting value (duplicated many times) into a new column.
...
There are various ways to select the columns to 'melt':
...
gap_medium <- gap_long_sep %>%
  pivot_wider(names_from = obs_type, values_fvalue)
head(gap_medium)
```
