...
out <- lapply(1:6, function(i) max(rnorm(100)))
```
...
* for parallelized versions see Module 9 or `?clusterApply`
...
A better approach to setting up the loop is this:
...
continents  # those using R <= 3.6 should see something odd; what happened ?!
...
usually good to start with something concrete but not general and
...
2) Take the for loop we looked at earlier and replace the loop with `lapply` using an index vector.
...
3) Consider the code where we used `sample()` in module 4.  Initialize a storage vector of 500 zeroes. Set up a bootstrap using a for loop, with 500 bootstrap datasets. Here are the steps within each iteration:
...
4) Write an R function that will take an input vector and set any value below a
...
5) Augment your function so that it checks that the input is a numeric vector
and return an error if not. (See the help information for `stop()`.)

6) Figure out what `invisible()` does and why it is useful when writing
functions. Use `invisible()` in your function from just above or in the
`sortByCol()` function.

### Advanced

7) Extend the `sortByCol()` function to handle an arbitrary number of columns on
...
8) Extend the `sortByCol()` function so that it can take either numeric indices or
...
9) Explore scoping in the following code. Explain why the result is 11 and not 3. Note that `funGenerator()` returns a function as the return object,
...
10) Here's a cool trick to pull off a particular element of a list of lists:
