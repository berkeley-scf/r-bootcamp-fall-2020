% R bootcamp, Module 9: Advanced topics
...
**POLL 9A**: Have you used object-oriented programming techniques in the past?

1) Yes, I've done it in R (S3, S4, ReferenceClasses, or R6)
2) Yes, in Python
3) Yes, in Java
4) Yes, in C++
5) Yes, in some other language
6) No
...
(Note: the next few slides on regression are the material we didn't have time to cover in Module 4.)
...
1. Markov chain Monte Carlo for fitting Bayesian statistical models
2. optimization

**POLL 9C**: Have you used R's parallel processing capabilities?
...
We'll use a new dataset here, which is a dataset of airline departure times (in particular delays) for all flights from SFO over a period of several years. We'll do a stratified analysis, fitting a GAM for each of the destination airports.
