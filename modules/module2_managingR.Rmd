...
install.packages(c('chron', 'colorspace', 'codetools', 'DBI', 'devtools',
                   ...
                   'mapproj', 'maps', 'munsell', 'patchwork', 'proftools', 'proto',
		   'purrr', 'R6', 'rbenchmark', 'RColorBrewer', 'Rcpp', 'reshape2',
		   'rJava', 'RSQLite', 'scales', 'spam', 'stringr', 'tidyr', 'xlsx',
                   'xlsxjars', 'xtable'))
```
...
To read and write from R, you need to have a firm grasp of where in the computer's filesystem (i.e., the hard drive) you are reading and writing from. 

```{r, eval=FALSE}
getwd()  # what directory will R look in?
# Linux/Mac specific
setwd('~/Desktop/r-bootcamp-fall-2020') # change the working directory
setwd('/Users/paciorek/Desktop') # absolute path
getwd()
setwd('r-bootcamp-fall-2020/modules') # relative path
setwd('../tmp') # relative path, up and back down the tree

# Windows - use either \\ or / to indicate directories
# setwd('C:\\Users\\Your_username\\Desktop\\r-bootcamp-fall-2020')
# setwd('..\\r-bootcamp-fall-2020')

# platform-agnostic
setwd(file.path('~', 'Desktop', 'r-bootcamp-fall-2020', 'modules')) # change the working directory
setwd(file.path('/', 'Users', 'paciorek', 'Desktop', 'r-bootcamp-fall-2020', 'modules')) # absolute path
getwd()
...
## Second try (only relevant if not using R 4.0 or later):
## ?read.table
...
## Third try:
## let's delve more deeply by looking at the levels of the categorical variable
levels(rta[ , 2])[c(1:5, 3041:3044)]
## alternatively we could look at the unique values in the character variable
unique(rta2[ , 2])[2780:2800]

## Once we realize that the 'x' values are messing things up, we can
## use `help(read.table)` to find an argument that allows us to fix things.
...
## Of course the code above assumes you know where to look for the 'x' value.
## Here's some code that uses functionality we haven't seen yet to figure that out:
uniqs <- unique(rta2[ , 2])
numUniqs <- as.numeric(uniqs)
uniqs[which(is.na(numUniqs))]
## `which()` picks out the index values corresponding to the elements that are TRUE
## so it allows us to figure out which of the elements of `uniqs` can't be converted to
## numeric values
```
...
We'll go through a short example of making changes to the r-bootcamp-fall-2020 repository. In this case you don't have permission to make changes so you'll just have to follow along as I do it. However, you could start your own repository and then you'd be able to do similar things.
